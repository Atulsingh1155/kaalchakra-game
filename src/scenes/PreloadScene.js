...
export class PreloadScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Load images - NO LEADING SLASH (keeping your preference)
    this.load.image('introBackground', 'assets/images/intro_bg.png');
    this.load.image('boy', 'assets/images/boy.png');
    this.load.image('grandmother', 'assets/images/grandmother.png');
    this.load.image('gameBackground', 'assets/images/game_bg.png');
    this.load.image('coin', 'assets/images/coin.png'); // This MUST load from file
    this.load.image('enemy', 'assets/images/enemy.png');
    this.load.image('game_bg2', 'assets/images/game_bg2.png');
    this.load.image('fire_ball', 'assets/images/fire_ball.png');
    ...
    // Handle load errors - DO NOT CREATE COIN FALLBACK
    this.load.on('loaderror', (fileObj) => {
      console.error('CRITICAL: Failed to load asset:', fileObj.key, fileObj.url);
      
      // COIN must come from file - no fallback
      if (fileObj.key === 'coin') {
        console.error('COIN IMAGE FAILED TO LOAD FROM:', fileObj.url);
        console.error('Check that public/assets/images/coin.png exists and is deployed');
        // DO NOT create fallback - let it fail so we can debug
      } else {
        // Handle other assets with the general fallback method
        this.createFallbackAsset(fileObj.key);
      }
    ...
    this.load.on('complete', () => {
      console.log('All assets loaded');
      // Create missing assets (excluding coin)
      this.createMissingAssets();
    });
  }
  ...
  createFallbackAsset(key) {
    ...
    switch(key) {
      ...
      case 'coin':
        // DO NOT create fallback coin
        console.error('COIN ASSET MISSING - This should never happen!');
        return; // Exit early, don't destroy graphics
  ...
  createMissingAssets() {
    ...
    // DO NOT create fallback coin - it MUST come from the image file
